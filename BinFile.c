...
void setBits(boardPosArray* pos_arr, BYTE* res, int j,int numOfPos) {
	res[0] = ((toNumeric(pos_arr->positions[j][0]) << 5) | ((pos_arr->positions[j][1] - 1) << 2));
	if (numOfPos > 1) {
		res[0] |= (toNumeric(pos_arr->positions[j + 1][0]) >> 1);
		res[1] = (toNumeric(pos_arr->positions[j + 1][0]) << 7) | ((pos_arr->positions[j + 1][1] - 1) << 4);
	}
	if (numOfPos > 2) {
		res[1] |= (toNumeric(pos_arr->positions[j + 2][0]) << 1) | ((pos_arr->positions[j + 2][1] - 1) >> 2);
		res[2] = ((pos_arr->positions[j + 2][1] - 1 ) << 6);
	}
	if (numOfPos > 3) {
		res[2] |= (toNumeric(pos_arr->positions[j + 3][0]) << 3) | (pos_arr->positions[j + 3][1] - 1);
	}
}
